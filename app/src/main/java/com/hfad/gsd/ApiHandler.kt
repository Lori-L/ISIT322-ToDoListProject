...
class ApiHandler :  AppCompatActivity() {
    ...
    //Uses a provided location type name to find its corresponding API ID
    //Collects the current latitude / longitude
    //Uses collected information to generate the API query string
    fun generateQueryString(chosenLocation: String) : String {
        //1. takes in a location name
        //2. identifies the location's ID in the API
        chosenLocationIndex = locationNames.indexOf(chosenLocation)
        chosenLocationId = locationIds[chosenLocationIndex]
        ...
        val currentCoords = listOf(40.83274904439099, -74.31632995605469)

        //4. creates the API query string
        val queryString = "https://spatial.virtualearth.net/REST/v1/data/Microsoft/PointsOfInterest?spatialFilter=nearby(${currentCoords[0].toString()},${currentCoords[1].toString()},5)&${'$'}filter=EntityTypeID%20eq%20%27$chosenLocationId%27&${'$'}select=EntityID,DisplayName,Latitude,Longitude,__Distance&${'$'}top=3&${'$'}format=json&key=AvGwd4j6II6ftFMSrxPBEjviEJSckGifQj_vxRQeqgp1x6SlaPuPMcMSHgAMB02w"
        return queryString
    }

    //Uses the querystring to create an API request contained within a JsonObjectRequest
    fun generateAPIRequest(queryString: String) : JsonObjectRequest {
        val request = JsonObjectRequest(Request.Method.GET,queryString, null, { res ->
            Log.d("Volley Sample", res.getJSONArray("data").toString())

            //extracts all qualifying locations' latitude and longitude coordinates
    ...
    fun returnCoordsList() : List<List<Double>>{
        return coordsList
    }
